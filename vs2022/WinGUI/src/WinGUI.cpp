...
LRESULT WINAPI HSLL::WinBase32::WinProc32(HWND hWnd, UINT Id, WPARAM wParam, LPARAM lParam)//窗口消息处理函数
{
	...
	case WM_NCHITTEST://响应拉伸与拖动
	{
		...
		{
			if (!mWin->CheckState(23) && mWin->CheckState(22))//主窗口没有标题栏并且主窗口响应拉伸(如果有标题栏或者不响应拉伸，则不需要处理拉伸逻辑)
			{
		...
		else if (PreResult == HTCLIENT && mWin->CheckState(27) && mWin->DraggingArea.Left * mWin->Width <= Pt.X && mWin->DraggingArea.Right * mWin->Width >= Pt.X &&
	...
	case WM_SIZING://等比例拉伸
		...
		if (bWin->CheckState(28) && bWin->CheckState(4))//保持窗口拉升比
		{
			mWin = (WinMain32*)bWin;
	...
	case WM_SETFOCUS:
		...
		if (bWin->CheckState(2) && bWin->CheckState(28))
			bWin->SetState(1, 1);
	...
	case WM_EXITSIZEMOVE:
		...
		if (bWin->CheckState(3) && bWin->CheckState(28))
			bWin->SetState(1, 1);
	...
	while (!pMain->CheckState(7))
	{
		if (GetMessageA(&msg, nullptr, 0, 0))
		{
			if (msg.message == WM_USER_WinCreate)
			{
				...
				pParam->pWind->SetState(29, 1);
			}
			TranslateMessage(&msg);
			DispatchMessageW(&msg);
		}
	}
	pMain->SetState(9, 1);//消息线程退出
	ExitThread(0);
}
...
{
	if (!CheckState(10))//未发出请求
	{
		SetState(10, 1);//标记已发出关闭请求

		if (CheckState(28))//主窗口	
			SendMessageA(hWnd, WM_CLOSE, (WPARAM)this, 0);
		else
		{
			pParent->RemoveChild(this);

			if (!pParent->CheckState(10))
				SendMessageA(hWnd, WM_CLOSE, (WPARAM)this, 0);
		}

		while (ChildList.size())
		{
			WinBase32* pFree = ChildList.back();
			pFree->Close();
			ChildList.pop_back();
		}
	}
	...
	:WinBase32(Width, Height, posX, posY)
{
	...
	NoClientWidth = 0;
	NoClientHeight = 0;
	SetState(2, 1);//窗口失去焦点时暂停绘制
}

void HSLL::WinMain32::Release()//请在窗口完全关闭后调用该函数
{
	while (!CheckState(8) || !CheckState(9))
		Sleep(2);
...
{
	if (CheckState(32) || !CheckState(5) || !CheckState(1))//不需要绘制
		return;
...
void HSLL::WinMain32::SetWinState(WinState State)
{
	...
	case HSLL::WinState_Normal:
		...
		if (CheckState(30) || CheckState(31))
			SendMessageA(hWnd, WM_SYSCOMMAND, SC_RESTORE, 0);
		else if (CheckState(21))
		{
			if (CheckState(23))
			{
...
{
	SetState(19, LeftTop);
	SetState(18, RightTop);
	SetState(17, LeftBottom);
	SetState(16, RightBottom);
	SetState(15, Top);
	SetState(14, Bottom);
	SetState(13, Left);
	SetState(12, Right);
...
void HSLL::WinMain32::SetStrechParams(BOOL SuspendDraw, BOOL FixedAspectRatio, float AspectRatio)//设置窗口拉升时的参数
{
	...
	SetState(4, FixedAspectRatio);
	SetState(3, SuspendDraw);
}

void HSLL::WinChild32::DrawTask(double Interval)//任务绘制
{
	if (CheckState(32) || !CheckState(5))//不需要绘制
		return;
